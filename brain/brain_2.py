...
class Simpleton(Brain):
    ...
    def chaseRedLight(self):
      print "Chasing Red Light"  
      i_left,rgb_left = self.lightDetectorRead("left")
      i_right,rgb_right = self.lightDetectorRead("right")
      
      RedRightUpper=rgb_left[0]+1
      RedRightLower=rgb_left[0]-1
      if rgb_left[0] > 254 and rgb_right[0] > 254:
          print "found red light"
          return 0, 0 #at light, wait
      if rgb_left[0] > rgb_right[0]:
          print "Light Left"
          return 0.1, 0.2 #turn left, into light
      if rgb_right[0] > rgb_left[0]:
          print "Light Right"
          return 0.1, -0.2 #turn right, into light
      if RedRightUpper > rgb_left[0] > RedRightLower:
    ...
    def determine_state(self):
      ...
      if (rgb_left[0]>0 or rgb_right[0]>0) and (rgb_right[1] < 1) and (rgb_left[1] < 1):
