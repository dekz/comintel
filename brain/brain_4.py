...
class Simpleton(Brain):
    ...
    def step(self):
      ...
      self.pos_tracker()
      if self.NSpos > 15 or self.EWpos > 15:
    ...
    def pos_tracker(self):
      compass = 60*self.compassRead() #convert to degrees
      compassrd = (compass * math.pi / 180) #convert to rads
      t_speed,r_speed = self.state_behavior_map[self.state]()
      NSmagnitude = t_speed * math.cos(compassrd)
      EWmagnitude = t_speed * math.sin(compassrd)

      self.NSpos = self.NSpos + NSmagnitude
      self.EWpos = self.EWpos + EWmagnitude

      print "compass = ", compass
      print "t_speed = ", t_speed
      print "N-S Total = ", self.NSpos
      print "E-W Total = ", self.EWpos
      print "N-S Positional Change = ", NSmagnitude
      print "E-W Positional Change = ", EWmagnitude

      return self.NSpos,self.EWpos
    ...
    def determine_state(self):
      ...
      if self.lap_counter > 1:
          if (-10 < self.NSpos < 10) and (-10 < self.EWpos < 10):
    ...
    def followCorner(self):
      ...
      if self.targetDist+0.1<self.sv[7]:
         ...
         return 0.05, -0.3 # turn right
      if self.targetDist-0.1>self.sv[7]:
         print "followCorner:: left"
         return 0.05, 0.3  # turn left
